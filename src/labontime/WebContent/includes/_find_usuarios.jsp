...
<div class="row">
	<div class="col-md-12">
		<div class="panel panel-info panel-dark" style="margin-bottom: 0px;">
			<div class="panel-heading"><span class="panel-title lbp">BÚSQUEDA DE USUARIOS</span></div>
			<div class="panel-body padding-xs-vr no-margin ">
				<form action="" id="find-${nameModule}">
					<div class="row" style="padding-bottom: 5px">
						
						<div class="col-md-2">
							<label class="control-label no-margin-b lb">PERFIL</label>
							<select class="form-control" name="fPerfil" id="fPerfil" class="form-control iForm">
								<option value="">[ Seleccione un perfil ]</option>
								<c:forEach items="${listaPerfiles}" var="perfil">
									<option value="${perfil.getPerfilId()}">${perfil.getNombre()}</option>
								</c:forEach>
							</select>
						</div>
						<div class="col-md-2">
							<label class="control-label no-margin-b lb">APELLIDO PATERNO</label>
							<input type="text" name="fPaterno" id="fPaterno" placeholder="Apellido Paterno" class="form-control input-sm iForm">
						</div>
						<div class="col-md-2">
							<label class="control-label no-margin-b lb">APELLIDO MATERNO</label>
							<input type="text" name="fMaterno" id="fMaterno" placeholder="Apellido Materno" class="form-control input-sm iForm">
						</div>
						<div class="col-md-2">
							<label class="control-label no-margin-b lb">NOMBRES</label>
							<input type="text" name="fNombre" id="fNombre" placeholder="Nombres" class="form-control input-sm iForm">
						</div>
						<div class="col-md-2">
...
init.push(function(){
	...
	$("#find-${nameModule}").validate({
		...
		submitHandler: function(form) {
			var query_string = $.param({"fPerfil": $("#fPerfil").val(),"fPaterno" : $("#fPaterno").val(),"fMaterno" : $("#fMaterno").val(),"fNombre" : $("#fNombre").val(),"fEstado": $("#fEstado").val(),"buscar": ""});
